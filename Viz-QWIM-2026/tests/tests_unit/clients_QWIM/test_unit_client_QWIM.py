...
class client_QWIM:
    ...
    def __init__(self, client_ID: str, first_name: str, last_name: str, client_type: Client_Type):
        """
        ...
        self.m_personal_info = pl.DataFrame(
            schema={
                "First Name": pl.Utf8,
                "Last Name": pl.Utf8,
                ...
                "Annuity Income Start Age": pl.Int64,
            }
        )
        ...
        self.logger = logging.getLogger("QWIM.Client")

    def update_personal_info(self, data: dict):
        """
        ...
        try:
            ...
            for field in numeric_fields:
                if field in filtered_data:
                    try:
                        ...
                        self.logger.warning(
                            f"Could not convert {field} to integer: {filtered_data[field]}"
                        )
            ...
            if "First Name" in data:
                self.m_first_name = data["First Name"]
            if "Last Name" in data:
                self.m_last_name = data["Last Name"]
    ...
    def update_assets(self, data: list):
        """
        ...
        try:
            ...
            if data:
                ...
                for asset in data:
                    ...
                    if "Taxable Assets" in asset:
                        try:
                            clean_asset["Taxable Assets"] = float(asset["Taxable Assets"])
                        except (ValueError, TypeError):
                            pass

                    if "Tax Deferred Assets" in asset:
                        try:
                            clean_asset["Tax Deferred Assets"] = float(asset["Tax Deferred Assets"])
                        except (ValueError, TypeError):
                            pass

                    if "Tax Free Assets" in asset:
                        try:
                            clean_asset["Tax Free Assets"] = float(asset["Tax Free Assets"])
                        except (ValueError, TypeError):
                            pass

                    # Optional fields
                    if "Asset Name" in asset:
                        clean_asset["Asset Name"] = str(asset["Asset Name"])

                    if "Asset Class" in asset:
                        clean_asset["Asset Class"] = str(asset["Asset Class"])
                    elif "Type" in asset:  # Backward compatibility
                        clean_asset["Asset Class"] = str(asset["Type"])
    ...
    def update_goals(self, data: list):
        """
            ...
            if data:
                ...
                for goal in data:
                    ...
                    for expense_type in [
                        ...
                        if expense_type in goal:
                            try:
                                clean_goal[expense_type] = float(goal[expense_type])
                            except (ValueError, TypeError):
                                pass
    ...
    def update_income(self, data: list):
        """
    ...
    def get_total_assets(self) -> float:
        ...
        try:
            ...
            totals = [
                asset_sums[0, f"total_{col.lower().replace(' ', '_')}"] for col in asset_columns
            ]
            total_assets = sum(t for t in totals if t is not None)
    ...
    def get_total_annual_income(self) -> float:
        ...
        try:
            ...
            # Calculate the grand total
            totals = [
                income_sum[0, f"total_{col.lower().replace(' ', '_')}"] for col in income_columns
            ]
            total_income = sum(t for t in totals if t is not None)
    ...
    def is_pension_inflation_indexed(self) -> bool:
        ...
        try:
            return bool(
                self.m_income.select(
                    pl.any("Annual Pension in Retirement is Inflation Indexed")
                ).item()
            )
        except Exception as e:
    ...
    def is_annuity_inflation_indexed(self) -> bool:
        """Check if annuity income is inflation indexed."""
        if self.m_income.is_empty():
            return False

        try:
            return bool(
                self.m_income.select(pl.any("Annual Annuity Income is Inflation Indexed")).item()
            )
        except Exception as e:
            self.logger.error(f"Error checking if annuity is inflation indexed: {e}")
            return False

    def is_other_income_inflation_indexed(self) -> bool:
        ...
        try:
            return bool(
                self.m_income.select(pl.any("Annual Other Income is Inflation Indexed")).item()
            )
        except Exception as e:
            self.logger.error(f"Error checking if other income is inflation indexed: {e}")
            return False
    ...
    def get_total_annual_expenses(self) -> float:
        ...
        try:
            ...
            # Calculate the grand total
            totals = [
                expenses_sum[0, f"total_{col.lower().replace(' ', '_')}"] for col in expense_columns
            ]
            total_expenses = sum(t for t in totals if t is not None)
    ...
    def get_essential_expenses(self) -> float:
    ...
    def get_important_expenses(self) -> float:
    ...
    def get_aspirational_expenses(self) -> float:
    ...
    def is_essential_expense_inflation_indexed(self) -> bool:
    ...
    def is_important_expense_inflation_indexed(self) -> bool:
    ...
    def is_aspirational_expense_inflation_indexed(self) -> bool:
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "client_QWIM":
        """
        ...
        """
        client = cls()
        client.m_client_ID = data.get("client_id")
        client.m_first_name = data.get("first_name")
        client.m_last_name = data.get("last_name")

        # Parse dates - updated to use m_creation_date and m_last_updated
        if "creation_date" in data:
            client.m_creation_date = datetime.fromisoformat(data["creation_date"])
        if "last_updated" in data:
            client.m_last_updated = datetime.fromisoformat(data["last_updated"])

        # Create dataframes from dictionary data
        if "personal_info" in data:
            client.m_personal_info = pl.DataFrame(data["personal_info"])  # Updated

        if "assets" in data:
            client.m_assets = pl.DataFrame(data["assets"])

        if "goals" in data:
            client.m_goals = pl.DataFrame(data["goals"])

        if "income" in data:
            client.m_income = pl.DataFrame(data["income"])
    ...
    def __str__(self) -> str:
        ...
        name = (
            f"{self.m_first_name} {self.m_last_name}"
            if self.m_first_name and self.m_last_name
            else self.m_client_ID
        )
