...
Feature: Results Tab — module API, naming conventions, and security helpers
  ...
  # ---------------------------------------------------------------------------
  # Naming convention — input IDs
  # ---------------------------------------------------------------------------

  @shiny_tab_results @naming @smoke
  Scenario: All Results tab input IDs follow the hierarchical convention
  ...
  @shiny_tab_results @naming
  Scenario: All Results tab output IDs follow the hierarchical convention
    Given the shiny_tab_results source code is loaded
  ...
  Scenario: A well-formed PDF filename is accepted
    When I sanitize the filename "QWIM_Report_2024.pdf"
    Then the sanitization result should indicate "valid"
    And the sanitized filename should not be empty

  @shiny_tab_results @sanitize @smoke
  Scenario: A filename with spaces is accepted
    When I sanitize the filename "Annual Report 2024.pdf"
    Then the sanitization result should indicate "valid"

  @shiny_tab_results @sanitize
  Scenario: The return value is always a 3-tuple
    When I sanitize the filename "Test.pdf"
  ...
  Scenario: An empty filename is rejected
    ...
    And the sanitized filename should be empty

  @shiny_tab_results @sanitize @smoke
  Scenario: A filename with path traversal is rejected
    When I sanitize the filename "../../etc/passwd.pdf"
    Then the sanitization result should indicate "invalid"

  @shiny_tab_results @sanitize
  Scenario: A filename without .pdf extension is rejected
    When I sanitize the filename "Report_2024.docx"
    Then the sanitization result should indicate "invalid"
