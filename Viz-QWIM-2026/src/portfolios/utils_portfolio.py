...
portfolio: type[portfolio_QWIM] | None = None
try:
    from portfolios.portfolio_QWIM import portfolio_QWIM as portfolio
except ImportError:
    try:
        from portfolios.portfolio import portfolio  # type: ignore[import-not-found]
    except ImportError:
        logger.warning("Could not import portfolio. Make sure it's in your path.")
        logger.debug(f"Current sys.path: {sys.path}")
        portfolio = None  # type: ignore[assignment]  # fallback when module unavailable
...
def load_sample_etf_data(
    ...
    try:
        ...
        logger.warning(f"Could not read CSV with automatic parsing: {e}")
    ...
    if "Date" not in data.columns and date_columns:
        ...
        logger.info(f"Renamed column '{old_name}' to 'Date' for consistency.")
    ...
    if "Date" in data.columns:
        ...
        logger.debug(f"Date sample values: {date_sample}")
...
def load_portfolio_weights(
    ...
    if "Date" not in data.columns and date_columns:
        ...
        logger.info(f"Renamed column '{date_columns[0]}' to 'Date' for consistency.")
    ...
    if "Date" in data.columns:
        try:
            ...
            logger.warning(
                f"Could not parse 'Date' column as date: {e}. Continuing with string dates.",
            )

    # Add a validation check to make sure the Date column exists
    if "Date" not in data.columns:
        raise ValueError(
            f"Required column 'Date' is missing from the portfolio weights data: {filepath}",
        )
...
def calculate_portfolio_values(
    ...
    # Debug info
    logger.debug(f"Portfolio components: {components}")
    logger.debug(f"Price data columns: {price_data.columns}")
    ...
    for comp in components:
        if comp != "Date" and comp in price_data.columns:
            ...
            logger.warning(f"Ignoring numeric component '{comp}' which is likely an index column")
        else:
            logger.warning(f"Component '{comp}' not found in price data columns")
    ...
    logger.debug(f"Using these components: {components}")
        ...
        for component in components:
            ...
            try:
                if prev_prices[component] > 0:
                    ...
                    # Apply weight to component return if exists in weights
                    if component in weight_row:
                        portfolio_return += component_return * weight_row[component]
                    else:
                        # Try with string conversion if numeric columns involved
                ...
                logger.warning(f"Error calculating return for {component}: {e}")
                # Skip this component
                continue
    ...
    # Add portfolio values to result DataFrame
    result = result.with_columns(pl.Series("Portfolio_Value", portfolio_values))
    ...
    if result.shape[0] == 0:
        logger.warning("No portfolio values could be calculated. Returning empty DataFrame.")
...
def create_benchmark_portfolio_values(portfolio_values: pl.DataFrame) -> pl.DataFrame:
    ...
    if dates and isinstance(dates[0], str):
        ...
        if not format_found:
            ...
            logger.warning("Could not parse dates. Using sequential indices for time periods.")
            date_objects = []
    else:
...
def visualize_portfolio_weights(
    ...
    try:
        ...
        logger.warning("Visualization requires matplotlib and seaborn to be installed.")
    ...
    try:
        ...
        try:
            ...
            if not etf_file.exists():
                logger.warning(f"ETF data file not found at {etf_file}")
                logger.warning("Please ensure this file exists before continuing.")

            if not weights_file.exists():
                logger.warning(f"Portfolio weights file not found at {weights_file}")
                logger.warning("Please ensure this file exists before continuing.")
            ...
            logger.info(f"Number of ETFs: {sample_portfolio_obj.get_num_components}")
            logger.info("\nSample Portfolio Weights (first 3 rows):")
            logger.info(sample_portfolio_obj.get_portfolio_weights().head(3))

            logger.info("\nETF Data (first 3 rows):")
            logger.info(sample_data_etfs.head(3))

            logger.info("\nPortfolio Values (first 3 rows):")
            logger.info(portfolio_values.head(3))
            ...
            logger.info(f"Benchmark portfolio values saved to: {benchmark_path}")
            ...
            try:
                ...
                logger.warning(f"Could not visualize weights: {e}")
            ...
            if missing_components:
                logger.warning(
                    f"These components are missing in the ETF data: {missing_components}",
                )
                logger.info(f"Available ETF columns: {etf_columns}")
                ...
                if suggestions:
                    ...
                    for comp, matches in suggestions.items():
                        logger.info(f"  '{comp}' might match: {matches}")
                ...
                logger.info("All components are present in ETF data.")

        except ImportError as e:
            logger.error(f"ImportError: {e}")
            logger.error("This may be because the 'portfolio' class could not be imported.")
            logger.error(
...
def create_custom_portfolio(
    ...
    return portfolio(  # pyrefly: ignore[bad-return]
